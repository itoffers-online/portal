...
final class PagePostOfferAtGroupTest extends OffersTestCase
{
    public function test_posting_offer_to_facebook_group() : void
    {
        ...
        $this->offers->module()->handle(new SetFacebookChannel(
            $specialization->slug(),
            'page_id',
            'page_token',
            'group_id',
        ...
        $this->offers->module()->handle(new PagePostOfferAtGroup($offer->id()->toString(), 'This is offer message for facebook'));

        $this->assertInstanceOf(FacebookPost::class, $this->offers->module()->facebookPostQuery()->findFacebookPost($offer->id()->toString()));
    }
